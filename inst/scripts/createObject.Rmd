...
The information which connects the sample information from GEO with the SRA run id is downloaded from [SRA](http://www.ncbi.nlm.nih.gov/sra/?term=SRP042161) using the **Send to: File button**. Add the SRP ID to the end of the csv file name. 
...
dat<-datasbl$load_all_samples(tumors,new_cell_id=FALSE,tpm=FALSE)
m<-dat[["m"]]
meta<-dat[["meta"]]
rm(dat)
...
patel_glio_2014<-summarizedexperiment$SummarizedExperiment(assays=list(counts=m),  colData=DataFrame(meta))
#max(abs(summarizedexperiment$assays(patel_glio_2014)$counts[1:1000,1:10] - m[1:1000,1:10])) #sanity check
save(patel_glio_2014,file="data/patel2014gliohuman.rda")
```

Include TPMs as additional assay

```{r}
#load("data/patel2014gliohuman.rda")
load("../../data/patel2014gliohuman.rda")
pdata<-summarizedexperiment$colData(patel_glio_2014)
tumors<-unique(pdata$tumorName)
dat<-datasbl$load_all_samples(tumors,new_cell_id=FALSE,tpm=TRUE)
tpm<-dat[["m"]]
counts<-summarizedexperiment$assay(patel_glio_2014,"counts")
patel_glio_2014_x<-summarizedexperiment$SummarizedExperiment(assays=list(counts=counts,tpm=tpm), colData = pdata)
saveRDS(patel_glio_2014_x,file="data/patel2014gliohuman.rds")
#patel_glio_2014_y<-readRDS("../../data/patel2014gliohuman.rds")
```
